!初期値を入力して収束を見る
!出力は出力回数と、その時点での解が出力されるため、収束速度を見るのに訳に立つ
...
!Newton法
!多分このプログラムで採用している、前回の計算結果と新しい計算結果の差を利用したものが合っている
!f(x)に代入して0になるかどうかは、f値の判定であって、解の値に関する判定ではないのでバツ
!　→　判定はxをに関して行う必要がある
!　多分上記の方法は、無言及薄の収束で証明できそうだから勉強必要
!===============================================================================
