!fowrard mode automatic differentiation
...
program auto_derivative_p
    ...
    real(8) df, a, b, c, d, x
    integer :: input_file_number = 11, io

    open(input_file_number, file='input_ad.dat', status='old', action='read', iostat = io)
    if (io /= 0) stop 'Error opening file'

    read(input_file_number, *) a, b, c, d, x
